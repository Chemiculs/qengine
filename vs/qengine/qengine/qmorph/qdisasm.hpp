...
#pragma region Enumerations / Structures
			...
			typedef struct interrupt_permutator {
				BYTE current_state; // current permutation of the instruction
				BYTE previous_state; // previous permutation of the instruction
				BYTE original_state; // original instruction held at the address
...
#pragma region Permutation Functions
			...
			inline INT32 __fastcall get_permutation_index(BYTE original_instruction) {
			...
			inline instruction_permutations __fastcall get_permutation_instruction(INT32 index) {
		...
		public:
			...
			/* specify __fastcall should the compiler choose to optimize the argument as a pointer (to fit register) */
			inline void __fastcall instruction_permutation(interrupt_region& region) {
				DWORD protection{};

				if (!VirtualProtect(region.region_address, region.region_length, PAGE_EXECUTE_READWRITE, &protection))
					throw std::bad_variant_access();

				for (auto& instruction : region.region_instructions) {
					instruction_permutations next_permutation;
					auto index = instruction.iterator_seed;

					next_permutation = get_permutation_instruction(index);
					...
					//update the object
					instruction.previous_state = instruction.current_state;
					instruction.current_state = static_cast<BYTE>(next_permutation);
					instruction.iterator_seed = index;

					*reinterpret_cast<PBYTE>(instruction.instruction_address) = static_cast<BYTE>(next_permutation);
				}
			...
			bool __cdecl analyze_executable_sections() {
				...
				for (auto& section : executable_sections) {
					...
					for (size_t i = 0; i < instruction_count; ++i) {
						const auto& instruction = instructions[i]; // reference the addressof(instructions[i]) to speed things up)
						if (instruction.id == X86_INS_INT1 || instruction.id == X86_INS_INT3) {
							/*
							if (*reinterpret_cast<PWORD>(instruction.address) != static_cast<WORD>(0xCCCC))
								continue;
								*/
							interrupt_region region{};
							region.region_address = reinterpret_cast<void*>(instruction.address);
							region.region_parent_section = i;
							region.region_length = static_cast<uint32_t>(0x01);
							...
							do {
								...
								if (region_iteration_instruction == instruction_permutations::INT3 || region_iteration_instruction == instruction_permutations::INT1) {
									...
									interrupt_instruction.current_state = region_iteration_instruction;  //  current instruction ( = original)
									interrupt_instruction.previous_state = static_cast<BYTE>(STATE_NULL); //  previos instruction (does not exist)
									interrupt_instruction.original_state = region_iteration_instruction;  //  original instruction
									interrupt_instruction.instruction_address = reinterpret_cast<void*>(region_iterator); // address / pointer to instruction
									interrupt_instruction.iterator_seed = static_cast<byte>(distribution(generator));
			...
			inline bool __fastcall morph_executable_sections(bool free_memory = true) {
...
#pragma region PE Header Manipulation
			...
			inline bool __cdecl zero_dos_header(bool erase_stub = true) {
			...
			inline bool __cdecl scramble_dos_header(bool erase_stub = true) {
...
#pragma endregion
			...
			inline bool __cdecl zero_nt_header() {
			...
			inline bool __cdecl scramble_nt_header() {
...
#pragma endregion
			...
			inline bool __cdecl zero_information_sections() {
...
#pragma endregion
			...
			inline void __cdecl parse_main_module_headers() {
...
#pragma region Ctor / Dtor
			...
			inline __cdecl qsection_assembler() {
